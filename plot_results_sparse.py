...
parser.add_argument(
    ...
    default="/home/nadja/tomo_project/Results_nuts_sparse/",
)
parser.add_argument(
    "-w",
    "--weights_dir",
    ...
    default="/home/nadja/tomo_project/Results_nuts_sparse/Model_Weights",
)
parser.add_argument(
    "-noise_sigma",
    "--noise_sigma",
...
for sigma in [2, 3, 5]:
    ...
    if args.dataset == "Heart":
        path = args.outputdir + "/Test_Results"
...
#
    ...
    if i == 0:
        fig, axes = plt.subplots(3, 4, figsize=(8, 8), constrained_layout=False)
    ...
    for j in range(4):  # 4 columns
        ...
        print(i, flush=True)
        if j == 0 and i > 0:
            axes[i, j].imshow(noisy[8][20:-20, 20:-20], cmap="gray")
            axes[i, j].text(
                0.5,
                1.05,
                "noisy ($\\sigma$ = " + str(sigma) + ")",
        ...
        if j == 0 and i == 0:
            ...
            axes[i, j].imshow(
                clean[8][20:-20, 20:-20], cmap="gray", vmin=0, vmax=1
            )  # Adjusted index for clean
        elif j == 2:
            axes[i, j].imshow(
                result_sob_y[8][20:-20, 20:-20],
                cmap="gray",
                vmin=0,
    ...
    if args.dataset == "Heart":
        ...
        plt.savefig(
            os.path.join(
                "/home/nadja/tomo_project/Results_Noisier2Inverse_Heart_final/Plots_Paper",
                "Plot_all" + "_y.svg",
            )
        )
        plt.savefig(
...
for sigma in [2, 3, 5]:
    ...
    for f in files:
        print(f, flush=True)
        if "Sob" in f and "_sigma_" + str(sigma) in f:
            data = np.load(os.path.join(path, f, "output_reco_results_z.npz"))
            result_sob_z = data["output_reco_array"]
            clean = data["clean_test"]
            noisier = data["recos_test_z"]
        ...
        elif "Inference" in f and "_sigma_" + str(sigma) in f and "Sob" not in f:
            data = np.load(os.path.join(path, f, "output_reco_results_z.npz"))
            result_inf_z = data["output_reco_array"]
            clean = data["clean_test"]
            noisier = data["recos_test_z"]
        ...
        elif "_sigma_" + str(sigma) in f and "Inf" not in f and "Sob" not in f:
            print(f + " we load inference weights for " + str(sigma))
            data = np.load(os.path.join(path, f, "output_reco_results_z.npz"))
            result_z = data["output_reco_array"]
            clean = data["clean_test"]
    ...
    plt.plot(psnr_values_z, label="PSNR z" + str(np.mean(psnr_values_z)))
    ...
    # Create a figure and axes with specified size and no spacing
    if i == 0:
        fig, axes = plt.subplots(3, 4, figsize=(9,6), constrained_layout=False)
