...
class Walnut(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if clean_path.endswith(".png"):
            clean = np.array(
                cv.imread(clean_path, cv.IMREAD_GRAYSCALE), dtype=np.float16
            )
        ...
        if self.noise_type == "gauss":
            ...
            clean_sino = np.array(
                (create_noisy_sinograms(np.expand_dims(clean, 0), 512, 0)).squeeze(0)
            )
            noisy = np.asarray(
                add_correlated_noise(clean_sino, self.noise_intensity, self.noise_sigma)
            )

        elif self.noise_type == "gauss_image":
            clean /= np.max(clean, axis=(-1, -2))
            clean_new = np.copy(clean)
            ### we add noise to the image and then compute sinograms, so we have correlated noise in data domain
            noisy_image = add_gaussian_noise(clean_new, self.noise_intensity)
            noisy = np.array(
                (
                    create_noisy_sinograms(np.expand_dims(noisy_image, 0), 512, 0)
                ).squeeze(0)
            )
        ...
        if self.noise_type == "salt_and_pepper":
            clean, noisy, noisier = (
                ...
                self.transform(noisy),
                self.transform(noisier),
            )
            clean, noisy, noisier = (
                clean.type(torch.FloatTensor),
                noisy.type(torch.FloatTensor),
                noisier.type(torch.FloatTensor),
            )
