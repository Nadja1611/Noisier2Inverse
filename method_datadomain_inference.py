...
#%%
parser = argparse.ArgumentParser(description='Arguments for denoising network.',add_help=False)
parser.add_argument('-l','--loss_variant', type=str, 
                    help='which loss variant should be used', default= "DataDomain_NW_Data_MSE")
parser.add_argument('-alpha','--alpha', type=float, 
                    help='how much noisier should z be than y', default= 1)
parser.add_argument('-angles','--angles', type=int, 
                    help='number of prosqueuejection angles sinogram', default= 512)
parser.add_argument('-batch_size','--batch_size', type=int, 
                    help='number of prosqueuejection angles sinogram', default= 16)
parser.add_argument('-lr','--learning_rate', type=float, 
                    help='which learning rate should be used', default= 1e-5)
parser.add_argument('-noise_type','--noise_type', type=str, 
                    help='add correlated or uncorrelated noise', default= "uncorrelated")
parser.add_argument('-noise_intensity','--noise_intensity', type=float, 
                    help='how intense should salt and pepper noise be', default= 0.05)
parser.add_argument('-o', '--logdir', type=str, 
                    
                    help='directory for log files', default = "/home/nadja/nadja/tomo_project/LION/logs" )
parser.add_argument('-w', '--weights_dir', type=str, help='directory to save model weights', 
...
weights_dir = args.weights_dir + '/Noise_'+args.noise_type +'_' + str(args.noise_intensity)+ '_batchsize_' + str(args.batch_size) +'_' + 'Gaussian_Method_' + args.loss_variant + "_alpha_" + str(args.alpha) +"_learning_rate_" + str(args.learning_rate) + "_angles_" + str(args.angles)
...
class Noiser2NoiseRecon():
    def __init__(
        ...
        # Dataset
        self.train_dataset = Walnut(noise_type = self.noise, noise_intensity = self.noise_intensity, train=True)
        self.test_dataset = Walnut(noise_type = self.noise, noise_intensity = self.noise_intensity,  train=False)
        self.train_dataloader = DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, pin_memory = True, num_workers = 0)
        self.test_dataloader = DataLoader(self.test_dataset, batch_size=self.batch_size, shuffle=False, pin_memory = True, num_workers = 0)
    ...
    def forward(self,reconstruction):
       ...
       # if we use the method where network operates in reconstruction domain, we have to project images back again
        ...
        output_denoising_sino = self.projection_tomosipo(output_denoising, sino = self.angles)#.to(self.device)
    ...
    def compute_reconstructions(self,sinograms):
        ...
        Reconstructions = torch.zeros((sinograms.shape[0],self.folds, sinograms.shape[-2], sinograms.shape[-2]),device = sinograms.device)
        ...
        for i in range(sinograms.shape[0]):
                Reconstructions[i] = self.fbp_tomosipo(sinograms[i].unsqueeze(0).unsqueeze(0), translate = False, angle_vector = projection_indices, folds = 1)
        del(sinograms)
        gc.collect()
    ...
    def projection_tomosipo(self, img, sino, translate = False):
        ...
        geo = ctgeo.Geometry.parallel_default_parameters(
            image_shape=(img.shape[0],img.shape[-1],img.shape[-1]), number_of_angles=angles, translate = False
        ) 
        # 0.2: create operator:
    ...
    def fbp_tomosipo(self, sino, angle_vector=None, translate=False, folds = None):
        angles = sino.shape[-1]
        geo = ctgeo.Geometry.parallel_default_parameters(
            image_shape=(sino.shape[0],336,336), number_of_angles=angles, translate = False, angle_vector = angle_vector
        ) 
...
newpath = r'/home/nadja/tomo_project/LION/Results_Paper_new/Noise_'+args.noise_type +'_' + str(args.noise_intensity)+ '_batchsize_' + str(args.batch_size) +'_' + 'Gaussian_Method_' + args.loss_variant + "_alpha_" + str(args.alpha) +"_learning_rate_" + str(args.learning_rate) + "_angles_" + str(args.angles)
...
for epoch in range(N_epochs):
    ...
    for batch, data in enumerate(N2NR.train_dataloader):
        ...
        if args.noise_type == "salt_and_pepper":
            ...
            y = torch.tensor(create_noisy_sinograms(np.array(noisy[:].squeeze()), N2NR.angles, 0))  
            z = torch.tensor(create_noisy_sinograms(np.array(noisier[:].squeeze()), N2NR.angles, 0))
            clean, noisy, noisier = clean, noisy.to(N2NR.device), noisier.to(N2NR.device)
        else:
            ### if gauss noise is chosen, then the noise is added in the data domain, and that dataloader reads in the noisy sinos
            clean, y, z = data['clean'].squeeze(), data['noisy'].squeeze(), data['noisier'].squeeze()
            y = y.unsqueeze(1)
        ...
        if args.loss_variant == "DataDomain_MSE_Inference_Sobolev":
            ...
            loss = torch.nn.functional.mse_loss(output_sino.float(), 2*y.float().detach())
    ...
    if epoch % 2000 ==0:
        with torch.no_grad():
            ...
            plt.title("clean")
            plt.subplot(224)
            plt.imshow(torch.abs(output_sino[0,0].detach().cpu()-y[0,0].detach().cpu()), aspect = 'auto')
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(N2NR.test_dataloader):
             #### apply gassian noise a second time to make the sinogram even noisier
                if args.noise_type == "salt_and_pepper":
                    clean_test, noisy_test, noisier_test = data['clean'], data['noisy'], data['noisier']
                    y_test = torch.tensor(create_noisy_sinograms(np.array(noisy_test.squeeze()), N2NR.angles, 0))  
                    z_test = torch.tensor(create_noisy_sinograms(np.array(noisier_test.squeeze()), N2NR.angles, 0))
                    clean_test, noisy_test, noisier_test = clean_test.squeeze(), noisy_test.to(N2NR.device), noisier_test.to(N2NR.device)
                else:
                    clean_test, y_test, z_test = data['clean'].squeeze(),  data['noisy'].squeeze(), data['noisier'].squeeze()
                ...
                for i in range(len(clean_test)):
                    ...
                    ssim_y_value = torch.tensor(skimage.metrics.structural_similarity(output_reco_y_np, ims_test_np, data_range=data_range)).to(device)
                    ssim_z_value = torch.tensor(skimage.metrics.structural_similarity(output_reco_z_np, ims_test_np, data_range=data_range)).to(device)

                    # Compute PSNR for y and z
                    psnr_y_value = torch.tensor(skimage.metrics.peak_signal_noise_ratio(output_reco_y_np, ims_test_np, data_range=data_range)).to(device)
                    psnr_z_value = torch.tensor(skimage.metrics.peak_signal_noise_ratio(output_reco_z_np, ims_test_np, data_range=data_range)).to(device)
            ...
            all_ssim_y = torch.cat((all_ssim_y, torch.tensor([torch.mean(torch.tensor(ssim_y))])))
            all_ssim_z = torch.cat((all_ssim_z, torch.tensor([torch.mean(torch.tensor(ssim_z))])))

            all_psnr_y = torch.cat((all_psnr_y, torch.tensor([torch.mean(torch.tensor(psnr_y))])))
            all_psnr_z = torch.cat((all_psnr_z, torch.tensor([torch.mean(torch.tensor(psnr_z))])))
            ### get back
